...
describe('Sequence must match respective path', () => {
  test('must match the path', () => {
    ...
    expect(edgecaseSequence[3].getNextStep({})).toBe(FULL_NAME);

    expect(edgecaseSequence[4].url).toBe(FULL_NAME);
    ...
    expect(edgecaseSequence[9].getNextStep({})).toBe(CONTACT_DETAILS);

    expect(edgecaseSequence[10].url).toBe(CONTACT_DETAILS);
